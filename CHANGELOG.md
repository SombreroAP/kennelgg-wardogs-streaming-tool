...
- **Twitch stream markers for every clip.** Every clip saved while you are live - kill-feed, chat, voice, hotkey or dock - puts a marker on your Twitch VOD named after it, so the VOD can be scrubbed from moment to moment (Twitch shows them on the video's timeline and in the Highlighter). The Twitch login needs one more permission for this: log in to Twitch again once (Settings, Clips & replays, Twitch clips), and the settings say so until you do. The account logged in must be the channel or one of its editors. It works with Twitch clips turned off.
- **YouTube chapters for the VOD.** Every clip saved while streaming becomes a line such as "1:02:14 Triple kill with the Galil", timed from when the stream started. When the stream stops, the list ("0:00 Start" first, lines closer than 10 s dropped, as YouTube requires) is saved next to your clips, and the dock's menu has **Copy YouTube chapters**: paste it into the VOD's description and YouTube makes chapters of it. The same times fit a Twitch VOD exported to YouTube.
- Fixed: changing a Settings switch that ClipHound acts on (chat replays among them) reached ClipHound only at the next unrelated change. It is sent at once now.
