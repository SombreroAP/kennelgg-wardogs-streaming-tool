...
| Replay stinger, bar, image, leaderboards (0.22.0) | `data/overlay/stinger.html` (bridge msg `stinger` in/out; covered 480-760 ms; `?t=&dir=` freezes a frame for previews) driven by `Engine::sendStinger`/`endReplay` (replay shown / stopped `kStingerCoverMs` after the stinger starts; seek pre-rolled by it). `session.html` places itself (state.session.overlay on/pos tl|bc/mode always|pop) and animates. `stats-image.*` + `ui/stats-dialog.*` render 1920x1080 on `data/stats/bg1-15.jpg`; fonts must be loaded by absolute path. Leaderboards: `statsConsent` 0/1/2, queue `plugin_config/.../stats-queue.jsonl`, POST https://kennel.gg/api/stats/{session,delete} = kennel-ops `tools/stats_server.py` (systemd kennel-stats, 127.0.0.1:8720, /var/lib/kennel-stats) |
